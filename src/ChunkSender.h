...
public:
	...
	enum eChunkPriority
	{
		E_CHUNK_PRIORITY_LEGENDARY = 0,
		E_CHUNK_PRIORITY_HIGH,
...
protected:
	...
	struct sChunkQueue
	{
		...
		bool operator <(const sChunkQueue & a_Other) const
		{
			...
			∴ return true (affirm we're "smaller", and get pushed down) only if our priority is bigger than theirs (they're more urgent)
			*/
